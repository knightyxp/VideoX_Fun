...
def parse_args():
    parser = argparse.ArgumentParser(description="Video-to-video SFT testing from JSON task list with parallel inference")
    ...
    parser.add_argument("--transformer_path", type=str, required=True, help="Path to SFT transformer checkpoint (pt or safetensors)")
...
def main():
    ...
    if rank == 0:
        print(f"Running parallel SFT inference with {world_size} GPUs")
        print(f"Using seed: {args.seed}")
        print(f"SFT transformer checkpoint: {args.transformer_path}")
    ...
    # Load SFT transformer checkpoint (keep variable name as transformer_path)
